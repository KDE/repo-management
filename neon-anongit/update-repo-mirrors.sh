...
if [ -e /tmp/update_neon_anongit.txt ]; then
  echo "Lock exists"
  exit 1;
else
  touch /tmp/update_neon_anongit.txt
fi
...
then
  ...
  rm /tmp/update_neon_anongit.txt
  exit 1
fi
...
for line in `cat ~/projects-to-anongit.list`; do
    ...
    if [ -e $bname -a -e $bname/HEAD ]
      then
        ...
        rm -rf $bname
        git clone --mirror git://git.neon.kde.org/$line $bname
        cd $bname
        git fsck
        git config transfer.fsckObjects true
        if [ $? -ne 0 ]
          then
            echo "Fresh mirror clone of git://git.neon.kde.org/$line failed git fsck, not following through with rest of update" | mail -r "sysadmin-systems@kde.org" -s "WARNING: git fsck problem on $hostname" sysadmin-systems@kde.org
            rm /tmp/update_neon_anongit.txt
            exit 1
        fi
        git update-server-info
